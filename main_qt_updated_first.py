...
class TradingAppQt(QWidget):
 ...
 ############back end#############
    ...
    #     try:
    #         if hasattr(self, 'ib') and self.ib.isConnected():
    #             ticker = self.ib.ticker(self.vix_contract)
    #             if ticker:
    #                 price = ticker.last

    #                 if price is not None and not math.isnan(price):
    #                     self.current_vix = float(price)
    #                     return price
    #     except Exception as e:
    #         print(f"Error getting current VIX: {e}")
    #     return "N/A"

    # def get_yesterday_vix(self):
    #     try:
    #         if hasattr(self, 'ib') and self.ib.isConnected():
    #             vix_historical = self.ib.reqHistoricalData(
    #                 self.vix_contract, 
    #                 endDateTime= '',
    ...
    #             )
    #             if vix_historical:
    #                 self.yesterday_vix = float(vix_historical[0].close)
    #                 return vix_historical[0].close
    #     except Exception as e:
    #         print(f"Error getting yesterday VIX: {e}")
    ...
    #     try:
    #         if hasattr(self, 'ib') and self.ib.isConnected():
    #             ticker = self.ib.ticker(self.spx_contract)
    #             if ticker:
    #                 price = ticker.marketPrice()
    #                 if price is not None and not math.isnan(price):
    #                     self.current_spx = float(price)
    #                     return f"{price:.2f}"
    #                 price = ticker.last
    #                 if price is not None and not math.isnan(price):
    #                     return f"{price:.4f}"
    #     except Exception as e:
    #         print(f"Error getting current SPX: {e}")
    #     return "N/A"
    
    # def get_spx_ema(self,n):
    #     spx_historical = self.ib.reqHistoricalData(
    #         self.spx_contract,
    #         endDateTime= datetime.now(),
    #         barSizeSetting='1 min',
    #         durationStr=f'{n*60} S',
    #         whatToShow='TRADES',
    #         useRTH=True
    #     )
    #     if spx_historical:
    ...
    #         ]})
    #         spx_df['ema'] = spx_df['price'].ewm(span=n, adjust=False).mean()
    #         last_ema = spx_df['ema'].iloc[-1]
    #         self.spx_ema = last_ema
    #         return last_ema
        

    # def get_buy_sell_strike(self):
    #     buy_strike_price = round(self.current_spx / 5) * 5
    #     sell_strike_price = buy_strike_price  + 30
    #     today = datetime.now().strftime('%Y%m%d')
    #     self.buy_option_contract = Option(
    #         symbol='SPX',
    ...
    #         exchange='CBOE'
    #     )
    #     self.sell_option_contract = Option(
    #         symbol='SPX',
    #         lastTradeDateOrContractMonth=today,
    ...
    #         exchange='CBOE'
    #     )
    #     self.ib.qualifyContracts(self.buy_option_contract)
    #     self.ib.qualifyContracts(self.sell_option_contract)
    #     buy_ticker = self.ib.reqMktData(self.buy_option_contract, '', False, False)
    #     sell_ticker = self.ib.reqMktData(self.sell_option_contract, '', False, False)
    #     self.ib.sleep(0.3)
    #     buy_bid = buy_ticker.bid
    #     buy_ask = buy_ticker.ask

    #     sell_bid = sell_ticker.bid
    #     sell_ask = sell_ticker.ask

    #     self.buy_option_info = (buy_strike_price, buy_bid, buy_ask)
    #     self.sell_option_info = (sell_strike_price, sell_bid, sell_ask)
        
    #     self.buy_labels[1].setText(f"{buy_strike_price:.2f}")
    #     self.buy_labels[2].setText(f"{buy_bid:.2f}")
    #     self.buy_labels[3].setText(f"{buy_ask:.2f}")

    #     self.sell_labels[1].setText(f"{sell_strike_price:.2f}")
    #     self.sell_labels[2].setText(f"{sell_bid:.2f}")
    #     self.sell_labels[3].setText(f"{sell_ask:.2f}")


    #     call_spread_price_bid = self.buy_option_info[1] - self.sell_option_info[2]
    #     call_spread_price_ask = self.buy_option_info[2] - self.sell_option_info[1]
    #     self.call_spread_labels[2].setText(f"{call_spread_price_bid:.2f}")
    #     self.call_spread_labels[3].setText(f"{call_spread_price_ask:.2f}")

    #     midpoint = (call_spread_price_bid + call_spread_price_ask) / 2
    #     self.current_net_price = midpoint
    #     self.current_net_price_label.setText(f"Midpoint price: {self.current_net_price:.3f}")
...
async def fetch_data(ui,mode, loop):
    ...
    try : 
        ...
        async def strike_options_task(spx_price):
            ...
            nonlocal buy_strike_price,ticker_buy,ticker_sell
            # print(spx_price)
