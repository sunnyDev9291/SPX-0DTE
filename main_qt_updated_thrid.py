...
class TradingAppQt(QWidget):
    ...
    def init_ui(self):
        ...
        close_btn.clicked.connect(lambda: {self.close_application()})  
        ...
        for i, (text, value) in enumerate(labels_data):
            ...
            data_layout.addWidget(label_widget, i, 0, Qt.AlignLeft)  
            value_label = QLabel(value, self)
            value_label.setFont(QFont('Segoe UI', 14))
            if "SPX EMA" in text:
                self.spx_ema_value_label = value_label
            data_layout.addWidget(value_label, i, 1, Qt.AlignLeft) 
