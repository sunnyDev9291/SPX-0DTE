...
class TradingAppQt(QWidget):
    ...
    def update_ema_label(self):
        """Updates the SPX SMA label based on the period input."""
        period = self.ema_period.text()
        self.spx_ema_label.setText(f"SPX SMA in {period} min")
    ...
    def update_ema_prices(self):
        ...
        self.spx_ema_label.setText(f"SPX SMA in {ema_period} min")
...
async def get_highest_spx_price(ib,contract):
    
    date_str = datetime.now().strftime('%Y%m%d')
    print(date_str)
    bars = await ib.reqHistoricalDataAsync(
        contract,
        endDateTime=f'{date_str} 7:30:00',  # end time
    ...
    if bars:
        ...
        print("Highest SPX High from 9:30–10:30 =", highest_high)
        return highest_high
    return None
